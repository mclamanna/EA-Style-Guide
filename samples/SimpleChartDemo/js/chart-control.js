			var chartControl = (function() {
					...
					function clearInput(elem) {
						d3.select(elem.parentNode).selectAll('input').nodes()[0].value = "";
					}
					...
					function draw(parent) {
						...
						_controlContainer.node().innerHTML = 
							...
							+		'<button name="sortAlpha" class="btn btn-sm btn-outline-secondary sort-alpha sort-asc" type="button"><i class="fa fa-lg fa-sort-alpha-down sort-asc" aria-hidden="true"></i><i class="fa fa-lg fa-sort-alpha-up sort-desc" aria-hidden="true"></i></button>'
							+		'<button name="sortValue" class="btn btn-sm btn-secondary sort-value sort-desc" type="button"><i class="fa fa-lg fa-sort-amount-down sort-desc" aria-hidden="true"></i><i class="fa fa-lg fa-sort-amount-up sort-asc" aria-hidden="true"></i></button>'
							+		'<div class="form-group form-group-sm find inner-addon right-addon">'
							+			'<label class="control-label sr-only" for="find">Search</label>'
							+			'<input name="find" class="form-control form-control-sm typeahead" data-provide="typeahead" autocomplete="off" type="text" placeholder="search term"/>'
							+			'<i class="fa fa-sm fa-search text-secondary" aria-hidden="true"></i>'
							+		'</div>'
							+		'<button name="reset" class="btn btn-sm btn-outline-secondary reset" type="button" style="display:none"><i class="fa fa-lg fa-undo" aria-hidden="true"></i></button>'
